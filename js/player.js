            ...
            if (playlist.songs.length > 0 && isSelected) {
                ...
                songList.innerHTML = playlist.songs.map(song => `
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 5px 0; border-bottom: 1px solid rgba(0, 0, 0, 0.05);">
                        <div style="flex: 1;">
                            <div style="font-size: 14px;">${song.name}</div>
                            <div style="font-size: 12px; color: #888;">${song.artist || '未知歌手'}</div>
                        </div>
                        <button onclick="event.stopPropagation(); removeSongFromPlaylist('${name}', '${song.id}')" style="background: none; border: none; color: #ef4444; cursor: pointer; font-size: 16px;">×</button>
                    </div>
                `).join('');
